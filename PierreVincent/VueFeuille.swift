...
struct VueFeuille: View {
    ...
    var body: some View {
        ...
        .navigationTitle("")
        .toolbar {
            // Ordre voulu (de gauche à droite) :
            // 1. Création/suppression  2. Affichage  3. Import (Importer+Dossier)  4. Export
            ...
            // 2. Menu d'affichage : Galerie / Liste.
            ToolbarItem(placement: .primaryAction) {
                Menu {
                    Picker("Affichage", selection: $modeAffichage) {
                        Label("Galerie", systemImage: "square.grid.2x2").tag("icone")
                        Label("Liste", systemImage: "list.bullet").tag("liste")
                    }
                    .pickerStyle(.inline)
                } label: {
                    Label("Affichage",
                          systemImage: modeAffichage == "icone" ? "square.grid.2x2" : "list.bullet")
                }
            }

            // 3. Set Import : Importer + Ouvrir le dossier des données.
